...
def only_cpu_commands(func):
    def wrapper(message):
        if message.text == "/cpu":
            return func(message)
        else:
            bot.reply_to(message, "Comando inválido. Digite /cpu para jogar contra o computador.")
    return wrapper

@bot.message_handler(func=lambda message: message.text.lower() in ["cooperar", "trair"])
@only_cpu_commands
